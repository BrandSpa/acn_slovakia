...
function bs_get_posts($type = 'post', $paged = 1, $category = '') {
	...
	$posts = array_map(function($post) {
		...
		$post->post_image = str_replace('http:', get_post_meta($post->ID, 'image_square_key')) || str_replace('http:', '', wp_get_attachment_url($thumbId)); //str_replace('http:', '', wp_get_attachment_url($thumbId));
